...
| 配置项 | 说明 | 默认值 | 示例 |
|--------|------|--------|------|
| OpenList 服务地址 | 完整访问地址，必填 | 无 | `https://alist.example.com` |
| 用户名 | 登录用户名，必填 | 无 | `admin` |
| 密码 | 登录密码，必填 | 无 | |
| 上传根路径 | 文件存储的目录路径，可自定义 | 空 | `/zhouyi/blog` |
| Token 接口路径 | 登录接口路径，可自定义 | `/api/auth/login` | `/api/auth/login` |
