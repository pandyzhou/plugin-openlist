...
- 修复插件前端模块未正确注册导致文件同步 Tab 不显示的问题
- 修复存储策略列表 API 路径错误导致加载失败
- 文件同步改为左侧菜单「工具」下的独立页面，不再延迟加载
- 补全 plugin.yaml 元数据（作者、homepage、issues 等）
- logo 改为本地打包，不依赖外部 URL

## [1.1.1]

- 优化上传性能：改为流式转发，不再将整个文件缓冲到内存，大幅减少上传延迟
